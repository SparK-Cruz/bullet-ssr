...
export default class Template {
    ...
    async init(initialData, root) {
        ...
        if (!isServer) {
            ...
            // fragment.append(...root.children);
            // if (fragment.children.length !== 0) {
            //     this.#parsed = fragment;
            // }
            root.innerHTML = '';
        }
    ...
    bindDataIfAttributes(doc) {
        [...doc.querySelectorAll(':not([data-ignore]) [data-if]')].forEach(node => {
            ...
            const bulletHole = document.createComment('');
            ...
            const remove = () => {
                ...
                visible = false;
                parent.replaceChild(bulletHole, current);
            };
            ...
            const insert = () => {
                ...
                visible = true;
                current = container.content.firstElementChild.cloneNode(true);
                parent.replaceChild(current, bulletHole);
        ...
        [...doc.querySelectorAll('[data-loop]:not([data-item])')].forEach(node => {
            ...
            const refreshList = (firstRun = false) => {
                ...
                (data ?? []).slice(existing.length).forEach((_, i) => {
                    ...
                    !firstRun && this.bindNode(itemNode);
                });
            };
