...
function renderTemplate(xhtml, owner, components) {
    const content = [...Document.parseHTMLUnsafe(xhtml)
        .querySelectorAll('template')].shift();
    return new Template(content, owner, components);
}

export default async function frag(name, xhtml, behavior = null, components = []) {
    behavior = behavior ?? {
        constructor() {
    ...
    class Base extends HTMLElement {
        ...
        async #fragInit() {
            ...
            const template = renderTemplate(xhtml, this, components);
