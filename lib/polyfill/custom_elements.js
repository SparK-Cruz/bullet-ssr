    ...
    #inject(window) {
        Object.keys(this.custom).forEach(key => {
            if (window.customElements.get(key)) {
                // Already defined for this window
                return;
            }

            /* FEED THE EVAL { */
            const Document = EnhanceDocument(window);
            const HTMLElement = EnhanceHTMLElement(window);
            ...
            window.customElements.define(key, localDefinition);
        });
    }
}
